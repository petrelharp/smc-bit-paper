...
(this should be taken into account when interpreting the resulting ARGs;
...
in the graph is not necessary as long as the ``locally
unary''~\cite{wong_general_2023,fritze2024forest} sections of common ancestor
nodes are retained, from the perspective of computing the likelihood.
We also show via a empirical examples that the likelihood is remarkably
robust to the presence of polytomies, and illustrate how it can be
...
% % % % % % % % % % % % % % % % % %
\subsection*{Robustness to partial information}
...
of this approach to incomplete information about recombination and 
coalescent events via a proof-of-concept example, in which we 
remove various levels of detail from a simulated ARG.

\begin{figure}
    \centering
    \includegraphics[width=0.8\textwidth]{figures/likelihood_surface}
    \caption{Log likelihood-ratio curves for ARGs containing
     varying degrees of information about the realised simulation.
    The ``Full ARG'' here is the result of an 
    \texttt{msprime} simulation of the SMC (100 diploid samples, genome length 1Mb, 
    $r=2.5\times10^{-8}$, $N_e=1\times10^4$) which contains complete 
    information about the simulated events 
    (9,378 nodes; 13,221 edges; 2,933 trees). 
    The ``RE nodes removed'' ARG is the 
    result of simplifying out all of the recombination nodes (3,332 nodes;
    6,901 edges; note that \texttt{msprime} uses distinct nodes for the 
    left and right parent of each recombination). The ``Fully simplified''
    ARG has all locally unary nodes removed from trees (3,332 nodes; 10,674
    edges). Each of these ARGs additionally has 5\% and 25\% of the 
    internal nodes removed to create polytomies.
    See the text for details on how the likelihood ratio values are computed.
    The ``Full ARG'' and ``RE nodes deleted'' lines coincide.
    \label{fig:lik-surface}}
\end{figure}

In Fig~\ref{fig:lik-surface} we begin with a realisation of the SMC as a ``full
ARG'', that is, containing complete information about every simulated event.
We then plot the likelihood computed by Eq.~\eqref{eq:full-lik} 
as we vary the recombination rate parameter $r$ (as a ratio
of the likelihood computed at the true value of $r$). Unsurprisingly, we can
see that the minimum of the likelihood-ratio curve when using the 
full simulated ARG is at the true value of the parameter.
What is surprising is that we obtain an indistinguishable curve when 
we remove all the recombination nodes from this ARG, and therefore 
lose information about the precise timing of recombination events.
In this case, the locally unary portions of coalescent nodes have given
all the information that we need about recombination events.
Removing all locally unary nodes in the fully simplified ARG does result
in a loss of information, and we can see that the minimum of the 
...
robust to the presents of polytomies in this example. For each of the three
...
% 0   poly_full_arg_smc_0.05  200.0   1025.565973 192.662462  3.153086
% 1   poly_unary_coal_0.05    200.0   429.315017  190.182253  4.372696
% 2   poly_fully_sfiied_0.05  200.0   0.000000    185.691468  6.393174
% 3   poly_full_arg_smc_0.25  200.0   779.846233  176.830549  10.307876
% 4   poly_unary_coal_0.25    200.0   323.054926  154.422588  21.104016
% 5   poly_fully_sified_0.25  200.0   0.000000    117.096174  33.241986
...
comparable. It is remarkable, however, how little effect introducing 
these polytomies has on the likelihood surface in this example. For 
the ARGs containing unary nodes, removing 25\% of the internal 
nodes has negligible effect, and even on the fully simplified 
ARG has a very minor effect on the minimum.
...
remarkably robust to incomplete information, tolerating the loss 
...
each recombination event is precisely pinpointed in time, 
contain a significant degree of redundancy in terms of capturing 
information about the generative process. 
It is possible, therefore, that explicitly estimating recombination 
events is not necessary to sample ARGs under the SMC (as ARGweaver
and SINGER do), potentially
leading to further improvements in the scalability of such methods.
...
UK Biobank, and passing over to a rigorous model-based
...
in the more distant past, when the assumptions required 
...
large-scale hueristic and MCMC sampling inference methods have their 
applications, and there may be synergy in their combination. 
Practical combination of methods will require cooperation at the 
data structure level, and the interoperability provided by the 
\texttt{tskit} library [something good].
