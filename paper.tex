...
The blue arrows show the familiar left-to-right logic of the SMC whereby
...
Surprisingly, although the SMC can be defined
in terms of a backwards-in-time Markov process,
the resulting tree-valued process along the genome is also Markov.
Linking this back to relationship between a graph and local trees view on
the ARG, then the distribution of the next tree within the series
...
endpoints of the involved lineages following common ancestry or recombination
...
\begin{figure}
    ...
    \caption{
        Graphical representation of the events associated with an edge $e(p,c,x_c,y)$.
...
$\exp(-r \times S)$, where $r$ is the recombination rate
