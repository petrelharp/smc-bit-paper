...
\begin{equation}\label{eq:full-lik}
    ...
    =
    (r)^{|E|-|N|+n_r} (\lambda)^{|E|-|N|+n_s} \prod_{(c, p) \in E} A_e(\theta) B_e(\theta) .
\end{equation}
...
\comment{This is \emph{very} nice!}
...
\comment{At first I thought we didn't need this next sentence now, but maybe
assigning-$\lambda$-and-$r$ to each edge makes it easier to recompute the likelihood
for slices?}
...
Rate changes along the genome can be incorporated similarly (see \ref{sec:slicing}).
...
time and/or along the genome. This tool therefore strengthens
...
The \tskit library provides a wide range
...
\comment{This next remark is totally not obvious, and a really nice observation;
maybe it needs a bit more explanation?}
\comment{Oh, I think it's not \emph{quite} true: if we slide at position $x$,
then any recombinations that happened at $x$ are now invisible, right?
So maybe you need to slice at non-integer positions, or do some bookeeping
about the number of edges with starts exactly at $x$?}
Remarkably, if we slice an ARG into disjoint pieces,
the likelihood of the full ARG
is the product of the likelihoods of each of the pieces.

Figure \ref{fig:3-arg-slices} demonstrates this idea
...
The result is reasonably accurate,
despite only using 10kb of sequence.
