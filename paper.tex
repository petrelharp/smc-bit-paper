...
We discuss the possibilities for inference that this opens. \revpoint{2}{3}
\end{abstract}
...
to a variety of questions in evolutionary biology~\citep{lewanski_era_2024,brandt_promise_2024,nielsen_inference_2025}. \revpoint{2}{4}
...
The first class of inference methods are Bayesian approaches \revpoint{2}{6}
...
scalability over a basis in a statistical model. \revpoint{2}{7}
...
omit these details. \revpoint{2}{8}
In many ways, the significant progress made in
...
A key problem facing the field is that the ARGs estimated by heuristic large-scale \revpoint{2}{10}
...
which does not contain sufficient detail for likelihood computation. \revpoint{2}{12}
The msprime
...
we mean that each node in the ARG represents a specific (ancestral) genome, \revpoint{2}{14}
and that the ARG specifies the genealogical tree describing how the samples are related
at each point on the genome. \revpoint{2}{15}
...
relationships between ancestral haplotypes, \revpoint{2}{16}
...
Other nodes (labelled by black numbers) are either samples
...
in the second tree, but not to node $c$, and so is unary. \revpoint{2}{21}
...
for instance, we know that $C$ happened somewhere between node $f$ and node $h$. \revpoint{2}{23}
...
of the SMC which also allows lineages with abutting genomic intervals \revpoint{2}{26}
...
and the total rate of coalescence at time $t$ is $\sum_{j} |C_j(t)|$.  \revpoint{1}{2}
...
and $\mathcal{A}_e$ is the total area (eligible links multiplied by length in time) \revpoint{2}{28}
of the edge.
The length in time of the edge is just $t_p - t_c$,
while the number of ``eligible links'' in the edge is the number of adjacent base pairs in the edge
...
between time $s$ and $t$ is given by $F(e, s, t) = \int_{s}^{t} I_{e}(z) dz$. \revpoint{1}{4}
...
Eq.~\eqref{eq:full-lik} allows us to compute a likelihood for \emph{any} \revpoint{2}{29}
...
(``Fully simplified''; 3,332 nodes; 10,674 edges, 2,933 trees)  \revpoint{2}{31}
...
ARG has only a very minor effect on location of the minimum.

Fig~\ref{fig:lik-surface} also shows results for ARGs inferred from the  \revpoint{2}{35}
...
this toy example at least, our likelihood function performs well \revpoint{2}{37}
...
removing all the unary nodes results in a significant bias \revpoint{2}{38}
...
strict input requirements for the classical Kuhner-Yamato-Felsenstein  \revpoint{2}{40}
formulation.
...
UK Biobank, passing over to a rigorous model-based approach \revpoint{2}{41}
...
large-scale heuristic and Bayesian sampling inference  \revpoint{2}{42}
...
allows us to sequentially recover (and reason about their differences) \revpoint{1}{5}
