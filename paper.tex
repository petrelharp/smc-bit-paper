...
% previous likelihood calculations on ARGsL Kuhner and Felsenstein (2000), ARGweaver
...
% improve this section, transition to modifications in the algorithm in case of 
% incomplete unary nodes, polytomies, ...
...
% section on how deviations from the ideal assumptions affect the algorithm

% keep track of last parent to identify recombination events
% recombination events beyond root: we keep track of
% parent of prior tree

% extent of lineages

% version of the algorithm with polytomies

% version of the algorithm in the absence of unary nodes

\subsection{Flexibility / general class of ARGs}

% highlight in what ways we can now compute likelihoods for a GENERAL class of ARGs
...
% something on the process backwards in time
We can readily incorporate changes in $N_e$ and $r$, in both dimensions (time and along the genome).

% HOW DO WE HIGHLIGHT THESE FEATURES: SUPPLEMENTS?
\subsubsection{Slicing the ARG in time}
% TO DO: give example why this might be useful.
...
past demography and selection (but see ). % some/many of these approaches 
...
% discuss accurracy:
% there will be scenarios where the assumptions made here are not met: e.g. polytomies
% yet we can still always compute a likelihood.
% we can alwyas associate a value, what remains to be verified what the information is in that
% case.

% explore the information in unary nodes
