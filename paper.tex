...
\begin{figure}
    \centering
    \includegraphics[width=0.8\textwidth]{figures/likratio.png}
...
Our final example concerns [TODO motivate this example; why is 
this interesting/useful? Presumably we don't have to jointly estimate
 the ARG with demography, but can do so post-hoc? But, couldn't 
you do that anyway if you had a full ARG using HKY?]
% slicing of the ARG
...
input, using a novel backwards-time derivation of the SMC.
...
Although we can associate a likelihood with any ARG in \texttt{tskit}
format, there are some caveats. By treating each edge independently we can
