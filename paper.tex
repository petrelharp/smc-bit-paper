...
both in time and along the genome. Yet the ease by which the CwR can be simulated,
and therefore the ease by which an ARG can be generated, is not not matched by 
the usability of the model for inference.
...
scalable ARG inference methods [references]. A real distinction can be made however 
between \argweaver, the first real breakthrough in scalable ARG inference, and the 
more recent methods. The ARGs sampled by \argweaver from the posterior distribution 
...
has to be limited to enable the usage of an HMM. This fully detailed inference holds 
for recombination, but also for migration events \cite{hubisz_2020_mapping}. 
All other methods, on the other hand, derive their scalability at least to a 
certain degree from the fact that 
the ARGs they infer are approximate structures. In particular, recombination is an 
emergent property of the inferred ARGs explaining tree topology changes going 
from one local tree to the next. The recombination events themselves are not 
explicitly inferred and are thus not part of the resulting data structure.
...
with the inevitable uncertainty associated with ARG reconstruction [Hayman, 2022]. 

Always events that cannot be known. Different ways of dealing with this.
Some approaches have embraced the approximate nature of the resulting ARG to for 
example highlight the inability to resolve certain splits (polytomies). 


others rigorous treatment of uncertainty (Arbores, ARGinfer): fully precise about recombination.

% onto ARG cannot be fully known
...
scale. 
As discussed in the introduction, the precise details of all 
events in these vast ARGs can never be known, and a data structure that enforces 
complete precision is therefore an impediment to progress.
...
quantify the uncertainty associated with ARG inference
