...
Three classes of ARG inference method have emerged as the focus of recent
work, and have all been successfully applied in different contexts. 
Parsimony
based methods~\citep{ignatieva_kwarg_2021,rasmussen_espalier_2022}
seek to minimise some aspect of the inferred ARG
(usually the number number of recombinations required), and work well for 
answering specific detailed questions about 
recombination~\citep[e.g.][]{ignatieva_ongoing_2022} when the expected
number of ancestral recombinations is small (tens to hundreds).
The second class of inference method is based on sampling ARGs statistically 
from a population genetics model~\citep{rasmussen_genome-wide_2014,
hubisz_mapping_2020,mahmoudi_bayesian_2022, deng_robust_2024}, and have 
been applied in several
contexts~\citep[e.g.][]{de_chimpanzee_2016,shriner_whole_2018,hejase_genomic_2020,
stankowski_genetic_2024}. These methods are well-suited to applications 
in which population genetics models are a good fit [TODO: Say this differently, 
I just want to distinguish from e.g. SARS-CoV-2 where pop gen models are 
basically meaningless]
in cases in which the expected number of recombinations is in the 
thousands to tens-of-thousands range (e.g., human datasets in which
we have X samples and a segment of Y Mb of chromosome [TODO quantify
from SINGER paper). 
The third class of inference method is based on heuristics
and approximate models and focus on large scale inference,
where the expected number of recombinations is in the millions or
tens of millions [TODO check numbers here]~\citep{kelleher_inferring_2019,
speidel_method_2019,zhang_biobank-scale_2023}.
Other methods specialised to particular applications have also 
recently been proposed~\citep{zhan_towards_2023,schaefer_ancestral_2021};
see~\cite{wong_general_2023} for a detailed review of ARG inference 
methods.

Statistically rigorous ARG sampling methods like
ARGweaver~\citep{rasmussen_genome-wide_2014} and
SINGER~\citep{deng_robust_2024} have many advantages. 
Firstly, ARGweaver has consistently out-performed other methods in terms 
...
of this?]. Heuristic, approximate inference methods like
Relate~\citep{speidel_method_2019}, tsinfer~\citep{kelleher_inferring_2019}
and ARG-Needle~\citep{zhang_biobank-scale_2023} on the other hand 
...
% and its refinement in the SMC'~\citep{marjoram_fast_2006},
% are the basis of many powerful inference 
% % TODO add some more recent stuff
% methods~\citep[e.g.]{li_inference_2011,paul_accurate_2011,schiffels_inferring_2014}.
% In particular, the SMC is the basis for ARG sampling methods
% like ARGweaver~\citep{rasmussen_genome-wide_2014} and 
% SINGER~\citep{deng_robust_2024}, which are the gold-standard 
% in terms of inference accuracy.
% ARGweaver has consistently out-performed other methods in terms 
% of accuracy in a variety of benchmarks~\citep{brandt2022evaluation},
% and, while SINGER has not been evaluated as thoroughly yet, it promises
% to be even more accurate as well as substantially more scalable 
% computationally~\citep{deng_robust_2024}.

% THE PROBLEM!!! OMG it's terrible!!!
Despite these recent advances in statistically rigorous ARG 
inference, however, there is still a gap of some three orders 
of magnitude between the sample-sizes that can be handled 
(hundreds) compared to the large-scale hueristic methods
like tsinfer and ARGneedle (hundreds of thousands).
[TODO working through a more positive narrative here, based on
the idea of combining the strengths of the approaches]
...
\emph{structures}~\citep{wong_general_2023} [WEAK, FIXME]. 
Specifically,
...
[Also: \cite{guo_recombination-aware_2022}]
...
genomic intervals may coalesce. (The SMC' additionally allows 
lineages with abutting genomic intervals to coalesce.)
