...
genetic material inherited by either one of a set of samples.
This ancestral material \citep{wiuf_ancestry_1999} carried by a
lineage can thus be summarized as $X_j(t) = [x_{j}, y_{j})$,
...
The newly formed lineage acquires the union of both intervals, % L is updated accordingly
and the original lineages are removed.
As a consequence, under the SMC common ancestry events are always
associated with coalescence events
along some fraction of the merging half-open intervals.
...
and $y = \sup\{ z \le max(y_j, y_k) : N(t,z) > 1\}$.
...
[TO DO: remark on  beyond local roots of the genealogy]
...
\begin{figure}[!ht]
\centering
% JK: I put in 0.9 here because the fig is too big and takes up more than 
% a page. This flushes all the figs to the end of the document. We should 
% either make it a bit smaller or reduce the caption.
...
An ARG can therefore be thought of either as a collection of inheritance relationships
...
Furthermore, these locally unary nodes enable
